...
namespace StudentsManagement.Services
{
    public class InstitutionService : IInstitutionService
    {
        ...
        public async Task<PaginatedResponse<InstitutionResponseDto>> GetAllPaginated(InstitutionQueryDto dto)
        {
            var query = InstitutionQueryValidator.Normalize(dto);

            var institutionsPaginated = await this._institutionRepository.GetAllPaginated(query);

            var items = institutionsPaginated.Items
                .Select(i => i.ToInstitutionResponseDto())
                .ToList();

            return new PaginatedResponse<InstitutionResponseDto>
            {
                Items = items,
                Meta = institutionsPaginated.Meta
            };
        }
