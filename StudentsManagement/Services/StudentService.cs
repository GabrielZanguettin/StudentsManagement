...
namespace StudentsManagement.Services
{
    public class StudentService : IStudentService
    {
        ...
        public async Task<PaginatedResponse<StudentResponseDto>> GetAllPaginated(StudentQueryDto dto)
        {
            var query = StudentQueryValidator.Normalize(dto);

            var studentsPaginated = await this._studentRepository.GetAllPaginated(query);

            var items = studentsPaginated.Items
                .Select(s => s.ToStudentResponseDto())
                .ToList();
