...
namespace StudentsManagement.Services
{
    public class SubjectService : ISubjectService
    {
        ...
        public async Task<PaginatedResponse<SubjectResponseDto>> GetAllPaginated(SubjectQueryDto dto)
        {
            var query = SubjectQueryValidator.Normalize(dto);

            var subjectsPaginated = await this._subjectRepository.GetAllPaginated(query);

            var items = subjectsPaginated.Items
                .Select(s => s.ToSubjectResponseDto())
                .ToList();

            return new PaginatedResponse<SubjectResponseDto>
            {
                Items = items,
